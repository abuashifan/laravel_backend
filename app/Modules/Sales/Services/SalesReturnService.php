...
return $q->orderByDesc('return_date')->orderByDesc('id')->get();
    ...
    public function update(SalesReturn $return, array $data): SalesReturn
    {
        if ($return->status !== 'draft') {
            throw ApiException::make('SALES_RETURN_NOT_EDITABLE', 'Sales return is not editable.', 422);
        }
        ...
        return DB::connection('tenant')->transaction(function () use ($return, $data, $lines) {
            $return->fill(array_merge($this->guardedForHeader($data), $this->totals($lines), ['revision_no' => (int) $return->revision_no + 1]))->save();
            $return->lines()->delete();
            $return->lines()->createMany($lines);
    ...
    {
        if (in_array((string) $deliveryOrder->status, ['cancelled', 'void', 'closed'], true)) {
            throw ApiException::make('SOURCE_NOT_CONVERTIBLE', 'Delivery order is not available for conversion.', 422);
        }
    ...
    {
        if ($return->status !== 'draft') {
            throw ApiException::make('INVALID_SALES_RETURN_STATUS', 'Invalid sales return status transition.', 422);
    ...
    public function post(SalesReturn $return): SalesReturn
    {
        if (! in_array($return->status, ['draft', 'approved'], true)) {
            throw ApiException::make('INVALID_SALES_RETURN_STATUS', 'Sales return cannot be posted.', 422);
        }
        $this->guardDate((string) $return->return_date);
        $this->assertMapping('sales.return');

        return DB::connection('tenant')->transaction(function () use ($return) {
            $return->load('lines');
            $journal = $this->journal($return);
            $return->journal_entry_id = $journal->id;
            $return->status = 'posted';
            $return->posted_by = auth()->id();
            $return->posted_at = now();
            $return->save();
            $this->inventoryIntegration->createSalesReturnIn($return);
    ...
    public function void(SalesReturn $return, ?string $reason = null): SalesReturn
    {
        if ($return->status === 'void') {
            throw ApiException::make('SALES_RETURN_ALREADY_VOID', 'Sales return already void.', 422);
        }
        ...
        return DB::connection('tenant')->transaction(function () use ($return, $reason) {
            ...
            if ($return->status === 'posted') {
                $this->restoreInvoice($return);
            }
            $return->status = 'void';
            $return->voided_by = auth()->id();
    ...
    private function normalizeReturnLines(array $lines): array
    {
        return array_values(array_map(fn (array $line, int $i): array => ['sales_invoice_line_id' => $line['sales_invoice_line_id'] ?? null, 'delivery_order_line_id' => $line['delivery_order_line_id'] ?? null, 'product_id' => $line['product_id'] ?? null, 'product_code' => $line['product_code'] ?? null, 'description' => $line['description'], 'quantity' => (float) $line['quantity'], 'unit_id' => $line['unit_id'] ?? null, 'unit_price' => (float) ($line['unit_price'] ?? 0), 'discount_amount' => (float) ($line['discount_amount'] ?? 0), 'tax_amount' => (float) ($line['tax_amount'] ?? 0), 'line_total' => (float) ($line['line_total'] ?? ((float) ($line['quantity'] ?? 0) * (float) ($line['unit_price'] ?? 0) - (float) ($line['discount_amount'] ?? 0) + (float) ($line['tax_amount'] ?? 0))), 'warehouse_id' => $line['warehouse_id'] ?? null, 'department_id' => $line['department_id'] ?? null, 'project_id' => $line['project_id'] ?? null, 'source_line_type' => $line['source_line_type'] ?? null, 'source_line_id' => $line['source_line_id'] ?? null, 'sort_order' => $line['sort_order'] ?? $i, 'metadata' => $line['metadata'] ?? null], $lines, array_keys($lines)));
    }

    private function validateReturnedQuantities(array $lines, ?SalesReturn $current = null): void
    {
        foreach ($lines as $line) {
            if ($line['sales_invoice_line_id']) {
                $invoiceLine = SalesInvoiceLine::query()->findOrFail((int) $line['sales_invoice_line_id']);
                $currentQty = $current ? (float) $current->lines()->where('sales_invoice_line_id', $invoiceLine->id)->sum('quantity') : 0.0;
                if ((float) $line['quantity'] > (float) $invoiceLine->quantity - (float) $invoiceLine->returned_quantity + $currentQty) {
                    throw ApiException::make('RETURN_QUANTITY_EXCEEDS_INVOICED', 'Return quantity exceeds invoiced quantity.', 422);
                }
            } if ($line['delivery_order_line_id']) {
                $deliveryLine = DeliveryOrderLine::query()->findOrFail((int) $line['delivery_order_line_id']);
                $currentQty = $current ? (float) $current->lines()->where('delivery_order_line_id', $deliveryLine->id)->sum('quantity') : 0.0;
                if ((float) $line['quantity'] > (float) $deliveryLine->quantity - (float) $deliveryLine->returned_quantity + $currentQty) {
                    throw ApiException::make('RETURN_QUANTITY_EXCEEDS_DELIVERED', 'Return quantity exceeds delivered quantity.', 422);
                }
            }
        }
    }
    ...
    private function totals(array $lines): array
    {
        ...
        $grand = array_sum(array_column($lines, 'line_total'));

        return ['subtotal_before_discount' => $subtotal, 'discount_total' => $discount, 'tax_total' => $tax, 'grand_total' => $grand];
    }

    private function updateInvoice(SalesReturn $return): void
    {
        if ($return->sales_invoice_id) {
            $invoice = SalesInvoice::query()->lockForUpdate()->find($return->sales_invoice_id);
            if ($invoice) {
                $invoice->returned_amount = (float) $invoice->returned_amount + (float) $return->grand_total;
                $invoice->balance_due = max(0, (float) $invoice->balance_due - (float) $return->grand_total);

                if ((float) $invoice->balance_due <= 0) {
                    $invoice->status = (float) $invoice->paid_amount > 0 ? 'paid' : 'returned';
                }
        ...
        foreach ($return->lines as $line) {
            if ($line->sales_invoice_line_id) {
                $invoiceLine = SalesInvoiceLine::query()->find($line->sales_invoice_line_id);
                if ($invoiceLine) {
                    $invoiceLine->returned_quantity = (float) $invoiceLine->returned_quantity + (float) $line->quantity;
                    $invoiceLine->save();
                }
            }
            if ($line->delivery_order_line_id) {
                $deliveryLine = DeliveryOrderLine::query()->lockForUpdate()->find($line->delivery_order_line_id);
                if ($deliveryLine) {
                    $deliveryLine->returned_quantity = (float) $deliveryLine->returned_quantity + (float) $line->quantity;
                    $deliveryLine->save();
                }
            }
        }
    ...
    private function restoreInvoice(SalesReturn $return): void
    {
        if ($return->sales_invoice_id && ($invoice = SalesInvoice::query()->lockForUpdate()->find($return->sales_invoice_id)) && $invoice->status !== 'void') {
            $invoice->returned_amount = max(0, (float) $invoice->returned_amount - (float) $return->grand_total);
            $invoice->balance_due = min((float) $invoice->grand_total, (float) $invoice->balance_due + (float) $return->grand_total);
            $invoice->save();
        } foreach ($return->lines as $line) {
            if ($line->sales_invoice_line_id && ($invoiceLine = SalesInvoiceLine::query()->lockForUpdate()->find($line->sales_invoice_line_id))) {
                $invoiceLine->returned_quantity = max(0, (float) $invoiceLine->returned_quantity - (float) $line->quantity);
                $invoiceLine->save();
            } if ($line->delivery_order_line_id && ($deliveryLine = DeliveryOrderLine::query()->lockForUpdate()->find($line->delivery_order_line_id))) {
                $deliveryLine->returned_quantity = max(0, (float) $deliveryLine->returned_quantity - (float) $line->quantity);
                $deliveryLine->save();
            }
        }
    }

    private function journal(SalesReturn $return): JournalEntry
    {
        $company = $this->tenantContext->company();
        if (! $company) {
            throw ApiException::make('COMPANY_NOT_FOUND', 'Company context not resolved.', 422);
...
return (int) $mapping->account_id;
    ...
    private function assertMapping(string $key): void
    {
        $mapping = AccountMapping::query()->where('mapping_key', $key)->where('is_active', true)->first();
        if (! $mapping?->account_id) {
            throw ApiException::make('MAPPING_REQUIRED', "Account mapping [{$key}] is required for this operation.", 422, ['account_mapping' => ["{$key} is not configured"]]);
        }
    }
}
