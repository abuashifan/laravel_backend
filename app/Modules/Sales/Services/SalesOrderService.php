...
class SalesOrderService
{
    ...
    public function create(array $data): SalesOrder
    {
        ...
        return DB::connection('tenant')->transaction(function () use ($company, $data) {
            ...
            if ((bool) ($data['has_down_payment'] ?? false) && ! empty($data['down_payment'])) {
    ...
    public function createFromQuotation(SalesQuotation $quotation, array $overrides = []): SalesOrder
    {
        if ($quotation->status === 'converted') {
            throw ApiException::make('QUOTATION_ALREADY_CONVERTED', 'Quotation already converted.', 422);
        }
    ...
    {
        return $this->transition($order, 'closed', 'closed_by', 'closed_at', ['confirmed', 'delivered', 'invoiced']);
    }
    ...
    public function refreshDeliveryStatus(SalesOrder $order): SalesOrder
    {
        ...
        $delivered = (float) $order->lines->sum('delivered_quantity');
        if ($delivered > 0) {
            $order->status = $delivered >= $total ? 'delivered' : 'partially_delivered';
            $order->save();
        }
...
return $order->refresh();
    ...
    private function transition(SalesOrder $order, string $status, string $userField, string $dateField, array $from): SalesOrder
    {
        if (! in_array($order->status, $from, true)) {
            throw ApiException::make('INVALID_SALES_ORDER_STATUS', 'Invalid sales order status transition.', 422);
        }
        $order->status = $status;
