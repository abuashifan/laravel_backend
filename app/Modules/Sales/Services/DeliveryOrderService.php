...
return $q->orderByDesc('delivery_date')->orderByDesc('id')->get();
    ...
    public function create(array $data): DeliveryOrder
    {
        ...
        $this->validateRemainingQuantities((array) $data['lines']);
    ...
    {
        if (! in_array($deliveryOrder->status, ['draft', 'ready'], true)) {
            throw ApiException::make('DELIVERY_ORDER_NOT_EDITABLE', 'Delivery order status is not editable.', 422);
        }
    ...
    public function createFromSalesOrder(SalesOrder $salesOrder, array $overrides = []): DeliveryOrder
    {
        ...
        $lines = $salesOrder->lines->map(function ($line) use ($requested, $requestedLines): ?array {
            ...
            if ($quantity <= 0 || $quantity > $remaining) {
                throw ApiException::make('DELIVERY_QUANTITY_EXCEEDS_REMAINING', 'Delivery quantity exceeds remaining sales order quantity.', 422);
            }
        ...
        if ($lines === []) {
            throw ApiException::make('SALES_ORDER_ALREADY_DELIVERED', 'Sales order has no remaining quantity to deliver.', 422);
        }
    ...
    public function markReady(DeliveryOrder $deliveryOrder): DeliveryOrder
    {
        return $this->transition($deliveryOrder, 'ready', 'ready_by', 'ready_at', ['draft']);
    }

    public function ship(DeliveryOrder $deliveryOrder): DeliveryOrder
    {
        return $this->transition($deliveryOrder, 'shipped', 'shipped_by', 'shipped_at', ['draft', 'ready']);
    }
    ...
    public function deliver(DeliveryOrder $deliveryOrder): DeliveryOrder
    {
        ...
        return DB::connection('tenant')->transaction(function () use ($deliveryOrder) {
            $this->validateRemainingQuantities($deliveryOrder->lines()->get()->toArray(), $deliveryOrder);
            $deliveryOrder->load('lines', 'salesOrder');
            $this->validateStockWarehousesForSalesLines($deliveryOrder->lines->toArray());
            foreach ($deliveryOrder->lines as $line) {
                if ($line->sales_order_line_id) {
                    $orderLine = SalesOrderLine::query()->findOrFail($line->sales_order_line_id);
                    $orderLine->delivered_quantity = (float) $orderLine->delivered_quantity + (float) $line->quantity;
                    $orderLine->save();
                }
            } $deliveryOrder->status = 'delivered';
            $deliveryOrder->delivered_by = auth()->id();
            ...
            if ($deliveryOrder->salesOrder) {
                $this->salesOrderService->refreshDeliveryStatus($deliveryOrder->salesOrder);
            }
...
return $deliveryOrder->refresh()->load('lines', 'customer', 'salesOrder');
    ...
    {
        $deliveryOrder->cancel_reason = $reason;

        return $this->transition($deliveryOrder, 'cancelled', 'cancelled_by', 'cancelled_at', ['draft', 'ready', 'shipped']);
    }

    public function void(DeliveryOrder $deliveryOrder, ?string $reason = null): DeliveryOrder
    {
        if (! in_array($deliveryOrder->status, ['draft', 'ready', 'shipped', 'delivered'], true)) {
            throw ApiException::make('INVALID_DELIVERY_ORDER_STATUS', 'Invalid delivery order status transition.', 422);
        }
        ...
        return DB::connection('tenant')->transaction(function () use ($deliveryOrder, $reason) {
            ...
            $deliveryOrder->status = 'void';
            $deliveryOrder->voided_by = auth()->id();
            $deliveryOrder->voided_at = now();
            $deliveryOrder->void_reason = $reason;
            $deliveryOrder->save();
    ...
    private function normalizeDeliveryLines(array $lines): array
    {
        return array_values(array_map(function (array $line, int $i): array {
            $normalized = ['sales_order_line_id' => $line['sales_order_line_id'] ?? null, 'product_id' => $line['product_id'] ?? null, 'product_code' => $line['product_code'] ?? null, 'description' => $line['description'], 'quantity' => (float) $line['quantity'], 'unit_id' => $line['unit_id'] ?? null, 'warehouse_id' => $line['warehouse_id'] ?? null, 'department_id' => $line['department_id'] ?? null, 'project_id' => $line['project_id'] ?? null, 'source_line_type' => $line['source_line_type'] ?? null, 'source_line_id' => $line['source_line_id'] ?? null, 'sort_order' => $line['sort_order'] ?? $i, 'metadata' => $line['metadata'] ?? null];
    ...
    private function validateRemainingQuantities(array $lines, ?DeliveryOrder $current = null): void
    {
        foreach ($lines as $line) {
            if (empty($line['sales_order_line_id'])) {
                continue;
            } $orderLine = SalesOrderLine::query()->findOrFail((int) $line['sales_order_line_id']);
            $currentQuantity = $current ? (float) $current->lines()->where('sales_order_line_id', $orderLine->id)->sum('quantity') : 0.0;
            if ((float) $line['quantity'] > (float) $orderLine->quantity - (float) $orderLine->delivered_quantity + $currentQuantity) {
                throw ApiException::make('DELIVERY_QUANTITY_EXCEEDS_REMAINING', 'Delivery quantity exceeds remaining sales order quantity.', 422);
            }
        }
    }

    private function transition(DeliveryOrder $deliveryOrder, string $status, string $userField, string $dateField, array $from): DeliveryOrder
    {
        if (! in_array($deliveryOrder->status, $from, true)) {
            throw ApiException::make('INVALID_DELIVERY_ORDER_STATUS', 'Invalid delivery order status transition.', 422);
        } $deliveryOrder->status = $status;
        $deliveryOrder->{$userField} = auth()->id();
        $deliveryOrder->{$dateField} = now();
        $deliveryOrder->save();
