...
class PurchaseReturnService
{
    ...
    {
        return PurchaseReturn::query()->with('lines.product', 'vendor', 'vendorBill', 'goodsReceipt')->findOrFail($id);
    }
    ...
    public function update(PurchaseReturn $return, array $data): PurchaseReturn
    {
        if ($return->status !== 'draft') {
            throw ApiException::make('PURCHASE_RETURN_NOT_EDITABLE', 'Purchase return is not editable.', 422);
        }
    ...
    public function createFromVendorBill(VendorBill $bill, array $overrides = []): PurchaseReturn
    {
        ...
        if ($bill->status === 'paid') {
            throw ApiException::make('PAID_BILL_RETURN_BLOCKED', 'Paid bill return is deferred for vendor credit workflow.', 422);
        }
    ...
    {
        if ($return->status !== 'draft') {
            throw ApiException::make('INVALID_PURCHASE_RETURN_STATUS', 'Invalid purchase return status transition.', 422);
    ...
    public function post(PurchaseReturn $return): PurchaseReturn
    {
        if (! in_array($return->status, ['draft', 'approved'], true)) {
            throw ApiException::make('INVALID_PURCHASE_RETURN_STATUS', 'Purchase return cannot be posted.', 422);
        }
    ...
    public function void(PurchaseReturn $return, ?string $reason = null): PurchaseReturn
    {
        if ($return->status === 'void') {
            throw ApiException::make('PURCHASE_RETURN_ALREADY_VOID', 'Purchase return already void.', 422);
        }
    ...
    {
        return array_values(array_map(fn (array $line, int $i): array => ['vendor_bill_line_id' => $line['vendor_bill_line_id'] ?? null, 'goods_receipt_line_id' => $line['goods_receipt_line_id'] ?? null, 'product_id' => $line['product_id'] ?? null, 'product_code' => $line['product_code'] ?? null, 'description' => $line['description'], 'quantity' => (float) $line['quantity'], 'unit_id' => $line['unit_id'] ?? null, 'unit_price' => (float) ($line['unit_price'] ?? 0), 'discount_amount' => (float) ($line['discount_amount'] ?? 0), 'tax_amount' => (float) ($line['tax_amount'] ?? 0), 'line_total' => (float) ($line['line_total'] ?? ((float) ($line['quantity'] ?? 0) * (float) ($line['unit_price'] ?? 0) - (float) ($line['discount_amount'] ?? 0) + (float) ($line['tax_amount'] ?? 0))), 'warehouse_id' => $line['warehouse_id'] ?? null, 'department_id' => $line['department_id'] ?? null, 'project_id' => $line['project_id'] ?? null, 'expense_account_id' => $line['expense_account_id'] ?? null, 'source_line_type' => $line['source_line_type'] ?? null, 'source_line_id' => $line['source_line_id'] ?? null, 'sort_order' => $line['sort_order'] ?? $i, 'metadata' => $line['metadata'] ?? null], $lines, array_keys($lines)));
    }

    private function validateReturnedQuantities(array $lines, ?PurchaseReturn $current = null): void
    {
        foreach ($lines as $line) {
            if ($line['vendor_bill_line_id']) {
                $billLine = VendorBillLine::query()->findOrFail((int) $line['vendor_bill_line_id']);
                $currentQty = $current ? (float) $current->lines()->where('vendor_bill_line_id', $billLine->id)->sum('quantity') : 0.0;
                if ((float) $line['quantity'] > (float) $billLine->quantity - (float) $billLine->returned_quantity + $currentQty) {
                    throw ApiException::make('RETURN_QUANTITY_EXCEEDS_BILLED', 'Return quantity exceeds billed quantity.', 422);
                }
            } if ($line['goods_receipt_line_id']) {
                $receiptLine = GoodsReceiptLine::query()->findOrFail((int) $line['goods_receipt_line_id']);
                $currentQty = $current ? (float) $current->lines()->where('goods_receipt_line_id', $receiptLine->id)->sum('quantity') : 0.0;
                if ((float) $line['quantity'] > (float) $receiptLine->quantity - (float) $receiptLine->returned_quantity + $currentQty) {
                    throw ApiException::make('RETURN_QUANTITY_EXCEEDS_RECEIVED', 'Return quantity exceeds received quantity.', 422);
                }
            }
        }
    }
    ...
    private function updateBill(PurchaseReturn $return): void
    {
        if ($return->vendor_bill_id) {
            $bill = VendorBill::query()->find($return->vendor_bill_id);
            if ($bill) {
                $bill->returned_amount = (float) $bill->returned_amount + (float) $return->grand_total;
                $bill->balance_due = max(0, (float) $bill->balance_due - (float) $return->grand_total);
                if ((float) $bill->balance_due <= 0) {
                    $bill->status = 'paid';
                } $bill->save();
            }
        } foreach ($return->lines as $line) {
            if ($line->vendor_bill_line_id && ($billLine = VendorBillLine::query()->find($line->vendor_bill_line_id))) {
                $billLine->returned_quantity = (float) $billLine->returned_quantity + (float) $line->quantity;
                $billLine->save();
            } if ($line->goods_receipt_line_id && ($receiptLine = GoodsReceiptLine::query()->find($line->goods_receipt_line_id))) {
                $receiptLine->returned_quantity = (float) $receiptLine->returned_quantity + (float) $line->quantity;
                $receiptLine->save();
            }
        }
    }

    private function restoreSource(PurchaseReturn $return): void
    {
        if ($return->vendor_bill_id && ($bill = VendorBill::query()->lockForUpdate()->find($return->vendor_bill_id)) && $bill->status !== 'void') {
            $bill->returned_amount = max(0, (float) $bill->returned_amount - (float) $return->grand_total);
            $bill->balance_due = min((float) $bill->grand_total, (float) $bill->balance_due + (float) $return->grand_total);
            $bill->status = $bill->paid_amount > 0 ? 'partially_paid' : 'posted';
            $bill->save();
        } foreach ($return->lines as $line) {
            if ($line->vendor_bill_line_id && ($billLine = VendorBillLine::query()->lockForUpdate()->find($line->vendor_bill_line_id))) {
                $billLine->returned_quantity = max(0, (float) $billLine->returned_quantity - (float) $line->quantity);
                $billLine->save();
            } if ($line->goods_receipt_line_id && ($receiptLine = GoodsReceiptLine::query()->lockForUpdate()->find($line->goods_receipt_line_id))) {
                $receiptLine->returned_quantity = max(0, (float) $receiptLine->returned_quantity - (float) $line->quantity);
                $receiptLine->save();
            }
        }
    }

    private function journal(PurchaseReturn $return): JournalEntry
    {
        $company = $this->tenantContext->company();
        if (! $company) {
            throw ApiException::make('COMPANY_NOT_FOUND', 'Company context not resolved.', 422);
    ...
    {
        if ($return->vendor_bill_id && ($bill = VendorBill::query()->find($return->vendor_bill_id))) {
            return $this->accountResolver->resolveBillPayableAccountId($bill);
        } $vendor = $return->relationLoaded('vendor') ? $return->vendor : Contact::query()->find($return->vendor_id);
        if (! $vendor) {
            throw ApiException::make('VENDOR_NOT_FOUND', 'Vendor not found.', 422);
        }

return $this->accountResolver->getPayableAccountId($vendor);
    }
